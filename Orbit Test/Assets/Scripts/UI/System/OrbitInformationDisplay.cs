...
public class OrbitInformationDisplay : MonoBehaviour
{
    ...
    */
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            textBoxes[0].text = Math.Round(BI.distanceFromObject, Universe.valueRounding).ToString();
            textBoxes[1].text = Math.Round(BI.ap, Universe.valueRounding).ToString();
            textBoxes[2].text = Math.Round(BI.pe, Universe.valueRounding).ToString();
            textBoxes[3].text = Math.Round(BI.speedOnOrbit, Universe.valueRounding).ToString();
